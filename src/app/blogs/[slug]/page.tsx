...
import { getHomePageSeo, YoastSeoData } from '@/services/wooCommerceApi';
...
const Blog = () => {
  ...
  const [homePageSeo, setHomePageSeo] = useState<YoastSeoData>({});
  ...
  useEffect(() => {
    ...
    const fetchSeoData = async () => {
      try {
        ...
        setHomePageSeo(seoData as YoastSeoData);
      } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {paginatedPosts.map((post) => (
              <Link key={post.id} href={`/blogs/news/${post.slug}`}>
